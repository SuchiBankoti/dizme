...
const Home = ({ dark }) => {
  ...
  useEffect(async () => {
    ...
    const userData = await fatchData(
      "https://portfolio-backend-30mp.onrender.com/api/v1/get/user/65b3a22c01d900e96c4219ae"
    );
    const skills = (await fatchData("/static/info.json")).skills;
    const {
      name,
      address,
      title: mainSkill,
    ...
    const skillsArr = skills.map((obj, i) => {
      const {
        image: { url: img },
      } = userData.user.skills[i];
      const { name } = obj;
      return {
        name,
        img,
                    ...
                    {data &&
                      ...
                      data.social.map((social, i) => (
                        <li key={i}>
                          <a href="#">
                            <i
                              className={`icon-${social.platform.toLowerCase()}-1`}
                            />
                          </a>
                        </li>
                      ))}
